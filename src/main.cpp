...
int main()
{
  ...
  double target_y = 0.0;
  double set_time = 4.0;
  double remaining_time = 2.0;
  long long time_us = 0;
  bool initialized = false;

  h.onMessage([&ukf,&target_x,&target_y, &set_time, &remaining_time, &time_us, &initialized](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          float dt = (meas_package_R.timestamp_ - time_us) / 1000000.0;	//dt - expressed in seconds
          time_us = meas_package_R.timestamp_;

          if(initialized){
            remaining_time -= dt;
            remaining_time = max(remaining_time, 0.0);
            remaining_time = min(remaining_time, set_time);
          }
          else{
            initialized = true;
          ...
          VectorXd future_loc;
          future_loc = ukf.Predict_future(remaining_time);

          target_x = future_loc[0];
          target_y = future_loc[1];
